...
void update_BSD_variables(void) {
    ...
    /*load_avg and recent_cpu of all threads are recalculated every second*/
    ...
    /*
     If the running thread is not idle thread, recent_cpu will increase by
     1.
     */
    if (thread_current() != idle_thread) {
        current_thread->recent_cpu = FIXED_POINT_ADD_INT(
                current_thread->recent_cpu, 1);
    }

    /*
     Recalculated priority of all the threads each tick.
     */
...
void wake_threads(void) {
    ...
    if (!list_empty(&sleep_thread_list)) {
        ...
        struct thread *thread_current = list_entry(e, struct thread,
                sleep_elem);
...
void thread_calc_priority(struct thread *thread, void *aux) {
    ...
    /*
     Adjust the calculated priority to the valid range.
     */
    if (result > PRI_MAX) {
        result = PRI_MAX;
...
void thread_calc_load_avg(void) {
    ...
    const int SIXTY = 60;
    int32_t coefficient = FIXED_POINT_DIV_INT(
            CONVERT_INT_TO_FIXED_POINT(FIFTEEN_NINE), SIXTY);
